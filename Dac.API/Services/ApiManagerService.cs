...
public class ApiManagerService : IApiManagerService
{
    ...
    public async Task<string> UpdatePatient(string id,Patient patient)
    {
        //todo** check existence and update...
        return await _patientRepository.UpdatePatient(id, Mapper.MapToPatientNode(patient));
    }
    ...
    public async Task<Patient> FetchPatientByID(string id, bool fetchAll)
    {
        try
        {
            ...
            if (!fetchAll)
            { //for updating patient no need to fetch everything
    ...
    public async Task<string> UpdateDoctor(string id, Doctor doc)
    {
        //todo** check existence and update...
        var d = await _doctorRepository.FetchDoctorByID(id) ?? throw new Exception("Doctor not found");

        return await _doctorRepository.UpdateDoctor(id, Mapper.MapToDoctorNode(doc));
    }
